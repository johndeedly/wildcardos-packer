...
log_text "Protect bind mount of /etc/resolv.conf (auto generated) and mnt/ (special privileges) in container and final system"
sed -i '0,/^#\?NoExtract.*/s//NoExtract = etc\/resolv.conf mnt\/\n\0/' /etc/pacman.conf
sed -i '0,/^#\?NoUpgrade.*/s//NoUpgrade = etc\/resolv.conf mnt\/\n\0/' /etc/pacman.conf
...
# Protect bind mount of /etc/resolv.conf in container (auto generated by systemd-resolved -> assume-installed & ignore filesystem)
log_text "Refresh and update everything except the filesystem package"
pacman -Syyuu --noconfirm --needed --noprogressbar --color=auto --assume-installed filesystem --ignore filesystem
