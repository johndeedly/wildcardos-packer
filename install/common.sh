...
if [[ ${TAGS[@]} =~ "archlinux" ]]; then
    function pacman_whenneeded() {
        log_text "Install package(s) ${*// /,}"
        LC_ALL=C yes | LC_ALL=C /usr/bin/pacman -Sy --disable-download-timeout --noconfirm --needed --noprogressbar --color=auto $*
    }
    function pacman_package_whenneeded() {
        log_text "Install package(s) ${*// /,}"
        LC_ALL=C yes | LC_ALL=C /usr/bin/pacman -U --disable-download-timeout --noconfirm --needed --noprogressbar --color=auto $*
    }
